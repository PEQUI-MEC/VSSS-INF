...
v4lcap::v4lcap() : fd(0), emulate_format(true), needs_convert(false), v4lconv(NULL), status(0), buffers() {}
...
bool v4lcap::start_capturing(enum memory_type mem_type, int nbuffers) {
	...
	if(format_src.fmt.pix.pixelformat != V4L2_PIX_FMT_RGB24) {
		needs_convert = true;
	} else {
		needs_convert = false;
	}
...
bool v4lcap::uninit_mmap() {
	...
	for(unsigned int i = 0; i < reqbuf.count; i++) {
		if(emulate_format) {
			if(-1 == v4l2_munmap(buffers.buffs[i].start, buffers.buffs[i].length))
				return false;
		} else {
	...
	if(!request_buffers_mmap(&reqbuf, 1))
		std::cout << "Can't request 1 mmap buffers.\n";

	if(!request_buffers_mmap(&reqbuf, 0))
		std::cout << "Can't request 0 mmap buffers.\n";
...
bool v4lcap::close_device() {
	...
	if(status & STREAM_ON)
		streamoff(V4L2_BUF_TYPE_VIDEO_CAPTURE);
...
bool v4lcap::get_standard(struct v4l2_standard * standard) {
	...
	if(xioctl(fd, VIDIOC_G_STD, &std_id) < 0)
		return false;
...
bool v4lcap::enum_control_menu(struct v4l2_querymenu * qmenu, struct v4l2_queryctrl & ctrl, int index, bool init) {
	...
	if((int) qmenu->index > ctrl.maximum)
		return false;
...
bool v4lcap::set_pixel_format(unsigned int pixel_format) {
	...
	if(!get_format(&fmt, V4L2_BUF_TYPE_VIDEO_CAPTURE))
		return false;
...
bool v4lcap::set_frame_interval(struct v4l2_fract & fract) {
	...
	if(!get_stream_parameter(&parm, V4L2_BUF_TYPE_VIDEO_CAPTURE))
		return false;
