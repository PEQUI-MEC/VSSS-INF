...
void Strategy::set_roles() {
	if(!transitions_enabled) {
		for(int robot_index = 0; robot_index < Robots::SIZE; robot_index++) {
			switch(Robots::get_role(robot_index)) {
				case ATTACKER:
					atk = robot_index;
					break;
	...
	for(int robot_index = 0; robot_index < Robots::SIZE; robot_index++) {
		if(robot_index == atk || robot_index == gk) continue;
		def = robot_index;
		break;
	}
...
void Strategy::set_role(int robot_index, int role) {
	Robots::set_status(robot_index, NORMAL_STATE);
	Robots::set_role(robot_index, role);
	Robots::set_velocity(robot_index, role);
}

bool Strategy::has_ball(int robot_index) {
	// Se o robô tem a bola pode levá-la pra frente
	double theta_i_ball = atan2(Ball.y - Robots::get_position(robot_index).y, Ball.x - Robots::get_position(robot_index).x);
	return distance(Robots::get_position(robot_index), Ball) < ABS_ROBOT_SIZE*1.5 //< robô está perto da bola
		   && (abs(Robots::get_orientation(robot_index) - theta_i_ball)*180/PI < 30 || abs(Robots::get_orientation(robot_index) - theta_i_ball)*180/PI > 150) //< bola está em uma das frentes
		   && Ball.x > Robots::get_position(robot_index).x + ABS_ROBOT_SIZE/2; // bola está na frente do atacante em relação ao gol inimigo
    ;
}
...
void Strategy::fixed_position_check(int robot_index) { // Para posição fixa
	if(Robots::get_fixedPos(robot_index) && (Robots::get_cmdType(robot_index) == Robots::CMD::VECTOR || Robots::get_cmdType(robot_index) == Robots::CMD::POSITION)) {
        Robots::set_velocity(robot_index, float(Robots::get_default_velocity(robot_index) * pow(distance(Robots::get_position(robot_index), Robots::get_target(robot_index))/(ABS_GOAL_TO_GOAL_WIDTH/4.0), 2)));
		...
		if(Robots::get_velocity(robot_index) > Robots::get_default_velocity(robot_index)) Robots::set_velocity(robot_index);
		if(Robots::get_velocity(robot_index) < 0.3) Robots::set_velocity(robot_index, 0.3f);

		if(distance(Robots::get_target(robot_index), Robots::get_position(robot_index)) <= fixed_pos_distance) {
			Robots::stop(robot_index);
		}
	}
}
...
void Strategy::collision_check(int robot_index) {
    if(Robots::get_fixedPos(robot_index)) return;

    // se é atacante e está dentro do gol
    if (Robots::get_role(robot_index) == ATTACKER &&
            Robots::get_position(robot_index).y > COORD_GOAL_UP_Y && Robots::get_position(robot_index).y < COORD_GOAL_DWN_Y &&
            Robots::get_position(robot_index).x < ABS_GOAL_SIZE_X + 0.75 * ABS_ROBOT_SIZE) {

        // verifica se está colidindo
        if(distance(Robots::get_position(robot_index), past_position[robot_index]) <= collision_radius) {
        ...
        if(collision_count[robot_index] >= max_collision_count) {
            ...
            if(distance(Robots::get_position(robot_index), upper_corner) > distance(Robots::get_position(robot_index), lower_corner) && Ball.y > COORD_GOAL_MID_Y){
                tempAngle = atan2(sin(past_transangle[robot_index]+(PI/2)),cos(past_transangle[robot_index]+(PI/2)));
            } else if (distance(Robots::get_position(robot_index), upper_corner) < distance(Robots::get_position(robot_index), lower_corner) && Ball.y < COORD_GOAL_MID_Y) {
                tempAngle = atan2(sin(past_transangle[robot_index]-(PI / 2)), cos(past_transangle[robot_index]-(PI / 2)));
            } else {
    ...
    else if(Robots::get_status(robot_index) != CORNER_STATE && Robots::get_status(robot_index) != ATK_PENALTI_STATE) {
		if(distance(Robots::get_position(robot_index), past_position[robot_index]) <= collision_radius &&
		...
		if(collision_count[robot_index] >= max_collision_count) {
			Robots::set_transAngle(robot_index, atan2(sin(past_transangle[robot_index]+PI),cos(past_transangle[robot_index]+PI)));
		}
	}
}
...
bool Strategy::is_near(int robot_index, cv::Point point) {
	return distance(Robots::get_position(robot_index), point) < fixed_pos_distance;
}

void Strategy::position_to_vector(int robot_index) {
	if(Robots::get_cmdType(robot_index) != Robots::CMD::VECTOR)
		Robots::set_transAngle(robot_index, atan2(double(Robots::get_position(robot_index).y - Robots::get_target(robot_index).y), - double(Robots::get_position(robot_index).x - Robots::get_target(robot_index).x)));
}
...
void Strategy::def_wait(int robot_index) {
    Robots::set_fixedPos(robot_index, true);
	if(distance(Robots::get_position(robot_index), Robots::get_target(robot_index)) <= fixed_pos_distance) {
	    Robots::look_at(robot_index, Ball);
	}
}

/*
void Strategy::pot_field_around(int robot_index) {
    Robots::set_usingPotField(robot_index, true);
	...
	double m2 = double(Robots::get_position(robot_index).y - Ball_Est.y)/double(Robots::get_position(robot_index).x - Ball_Est.x);
	...
	Robots::set_target(robot_index, targets_temp);
	Robots::set_transAngle(robot_index, potField(robot_index, targets_temp, BALL_ONLY_OBS));

	if(Robots::get_position(robot_index).x > Ball.x + ABS_ROBOT_SIZE)
		action1 = false;

	if(is_near(robot_index, targets_temp) || abs(phi) < beta/2 || action1) {
		v = cv::Point(Robots::get_position(robot_index).x - Ball.x, Robots::get_position(robot_index).y - Ball.y);
		...
		Robots::set_transAngle(robot_index, atan2(double(Robots::get_position(robot_index).y - Ball.y), - double(Robots::get_position(robot_index).x - Ball.x)));
		action1 = true;
	}
}
*/

void Strategy::crop_targets(int robot_index) {
    cv::Point target = Robots::get_target(robot_index);
	...
	Robots::set_target(robot_index, target);
}

void Strategy::atk_routine(int robot_index) {
	cv::Point target;
	cv::Point goal = cv::Point(COORD_GOAL_ATK_FRONT_X, COORD_GOAL_MID_Y);

	double m1 = double(goal.y - Ball.y)/double(goal.x - Ball.x);
	double m2 = double(Robots::get_position(robot_index).y - Ball.y)/double(Robots::get_position(robot_index).x - Ball.x);
	double phi = atan((m2 - m1) / (1 + m2 * m1));

	Robots::set_velocity(robot_index);

	if(Robots::get_status(robot_index) != ATK_PENALTI_STATE) {
		if((Robots::get_position(def).x < COORD_MID_FIELD_X // se o defensor tá atrás do meio de campo
			...
			||
			(has_ball(robot_index) && // atacante tem a posse de bola
			abs(phi)*180/PI < 30)) { // angulo com o gol tá bom
			Robots::set_status(robot_index, ATK_PENALTI_STATE); // chuta
		}
			...
			Robots::set_status(robot_index, CORNER_STATE); // a bola tá no corner, gira
		}
		else if(Ball.x < def_corner_line) { // se a bola tá antes da nossa linha x da área
			Robots::set_status(robot_index, DEF_CORNER_STATE); // a bola tá no nosso corner
		}
			...
			Robots::set_status(robot_index, SIDEWAYS); // sideways
		} else {
			Robots::set_status(robot_index, NORMAL_STATE); // senão, age normalmente
		}
	}

	switch(Robots::get_status(robot_index)) {
		case NORMAL_STATE:
			// leva a bola pro gol deles
			Robots::set_cmdType(robot_index, Robots::CMD::VECTOR);
			// pot_field_around(robot_index);
			// run(robot_index);
		...
		case SIDEWAYS:
			...
			Robots::set_cmdType(robot_index, Robots::CMD::POSITION);

			// empurra a bola no canto mesmo
			if(Ball.y > ABS_FIELD_HEIGHT - ABS_ROBOT_SIZE * 1.5) { // se a bola tá na lateral inferior
				if(distance(Robots::get_position(robot_index), Ball) > ABS_ROBOT_SIZE // se a bola tá longe do robô
					|| Ball.x >= Robots::get_position(robot_index).x + ABS_ROBOT_SIZE/1.5) { // ou se a bola não tá travada por ele
				    target = Ball;
				}
				else { // se a bola tá travada
					Robots::spin_counter_clockwise(robot_index, 0.8);
				}

				if(Ball.x < Robots::get_position(robot_index).x) { // se a bola tá atrás do atacante
				...
				if(distance(Robots::get_position(robot_index), Ball) > ABS_ROBOT_SIZE // se a bola tá longe do robô
					|| Ball.x >= Robots::get_position(robot_index).x + ABS_ROBOT_SIZE/1.5) { // ou se a bola não tá travada
                    target = Ball;
				}
				else { // se a bola tá travada
					Robots::spin_clockwise(robot_index, 0.8);
				}
		...
		case CORNER_STATE:
			...
			if(Ball.y < COORD_GOAL_MID_Y) { // se a bola tá acima do gol
				if(Ball.y < COORD_GOAL_UP_Y) { // acima ou abaixo da trave, escolher o giro para levar a bola para o gol ou para faze-la entrar
					if(Ball.x > Robots::get_position(robot_index).x && distance(Ball, Robots::get_position(robot_index)) < ABS_ROBOT_SIZE)
						Robots::spin_clockwise(robot_index, 0.8); // giro que leva a bola ao gol
					else
						target = Ball; // faz a bola travar no canto pra girar certo
				} else {
					if(Ball.y > Robots::get_position(robot_index).y && distance(Ball, Robots::get_position(robot_index)) < ABS_ROBOT_SIZE)
						Robots::spin_counter_clockwise(robot_index, 0.8); // giro para faze-la entrar
					else
						target = Ball;
				}
			} else {
				if(Ball.y > COORD_GOAL_DWN_Y) { // se a bola tá abaixo do gol
					...
					if(Ball.y > Robots::get_position(robot_index).y && distance(Ball, Robots::get_position(robot_index)) < ABS_ROBOT_SIZE)
						Robots::spin_clockwise(robot_index, 0.8); // giro para faze-la entrar
					else
						target = Ball;
				}
			}
		...
		case DEF_CORNER_STATE:
			...
			if(Ball.y > COORD_GOAL_MID_Y) {
				...
				if(distance(Ball, Robots::get_position(robot_index)) < ABS_ROBOT_SIZE && Ball.y > Robots::get_position(robot_index).y)
					Robots::spin_counter_clockwise(robot_index, 0.8);
				// if(distance(Robots::get_position(i), robots[i].target) <= fixed_pos_distance) robots[i].target = Ball;
				if(is_near(robot_index, target) && Ball.y > Robots::get_position(robot_index).y) {
					target = Ball;
				}
	...
	//crop_targets(robot_index);
}

void Strategy::def_routine(int robot_index) {
	cv::Point target;
	cv::Point goal = cv::Point(COORD_GOAL_ATK_FRONT_X, COORD_GOAL_MID_Y);
	...
	if(Robots::get_position(robot_index).x > COORD_MID_FIELD_X && abs(phi)*180/PI < 30) {
		Robots::set_status(robot_index, ATK_PENALTI_STATE);
	}
	else if(Robots::get_position(atk).x > Ball.x
		&& Ball.x < COORD_MID_FIELD_X
		&& distance(Robots::get_position(atk), Ball) > 3*ABS_ROBOT_SIZE) {
		Robots::set_status(robot_index, STEP_BACK);
	}
	else if(Ball.x < def_corner_line) {
		Robots::set_status(robot_index, DEF_CORNER_STATE);
	}
	else {
		Robots::set_status(robot_index, NORMAL_STATE);
	}
	...
	switch(Robots::get_status(robot_index)) {
		case NORMAL_STATE:
			...
			if(Ball_Est.x > corner_atk_limit) {
				...
				Robots::set_target(robot_index, target); // deve ser chamado antes do wait
				def_wait(robot_index);
			}
		...
		case STEP_BACK:
			...
			if(Ball.x > Robots::get_position(robot_index).x + ABS_ROBOT_SIZE) {
				Robots::set_target(robot_index, Ball);
			}
			else {
				// pot_field_around(robot_index);
				crop_targets(robot_index);
		...
		case DEF_CORNER_STATE:
			...
			if(Ball.y > COORD_GOAL_MID_Y) {
				...
				if(distance(Ball, Robots::get_position(robot_index)) < ABS_ROBOT_SIZE && Ball.y > Robots::get_position(robot_index).y)
					Robots::spin_counter_clockwise(robot_index, 0.8);
				// if(distance(Robots::get_position(robot_index), Robots::get_target(robot_index)) <= fixed_pos_distance) robots[robot_index].target = Ball;
				if(is_near(robot_index, target) && Ball.y > Robots::get_position(robot_index).y) {
					target = Ball;
				}
			} else {
		...
		case ATK_PENALTI_STATE:
			...
			Robots::set_cmdType(robot_index, Robots::CMD::VECTOR);
            Robots::kick(robot_index, Ball);
    }
}

void Strategy::gk_routine(int robot_index) {
	switch(Robots::get_status(robot_index)) {
		case NORMAL_STATE:
			Robots::set_fixedPos(robot_index, true);
			...
			if(distance(Ball, Ball_Est) > ABS_ROBOT_SIZE && Ball.x > Ball_Est.x && (Ball.x > COORD_BOX_DEF_X)) {
				...
				if(Ball.x > COORD_MID_FIELD_X) {
					...
					double approach = abs(Ball.y - pred_y) * (Ball.x - (Robots::get_position(robot_index).x + ABS_ROBOT_SIZE))/float(COORD_MID_FIELD_X - Robots::get_position(robot_index).x);
			...
			if(distance(Robots::get_position(robot_index), Ball) < ABS_ROBOT_SIZE*1.3) {
				if(Ball.y <= COORD_GOAL_UP_Y) {
					Robots::spin_clockwise(robot_index, 1.f);
				} else if(Ball.y >= COORD_GOAL_DWN_Y) {
					Robots::spin_counter_clockwise(robot_index, 1.f);
				}
			}

			if(distance(Robots::get_position(robot_index), Ball) < ABS_ROBOT_SIZE*1.3) {
				if(Ball.y <= Robots::get_position(robot_index).y) {
					Robots::spin_clockwise(robot_index, 1.f);
				} else if(Ball.y >= Robots::get_position(robot_index).y){
					Robots::spin_counter_clockwise(robot_index, 1.f);
				}
			}
			// chute pra tirar a bola da defesa
			if(abs(Ball.x - Robots::get_position(robot_index).x) <= ABS_ROBOT_SIZE && Ball.y < COORD_BOX_DWN_Y && Ball.y > COORD_BOX_UP_Y) {
				if (Ball.y <= Robots::get_position(robot_index).y) {
					target.y = 0;
					target.x = goalie_line; //  + ABS_ROBOT_SIZE/2
					Robots::set_velocity(robot_index, MAX_VEL);
				} else {
					target.y = 480;
					target.x = goalie_line; //  + ABS_ROBOT_SIZE/2
					Robots::set_velocity(robot_index, MAX_VEL);
			...
			if(distance(Robots::get_position(robot_index), Robots::get_target(robot_index)) < ABS_ROBOT_SIZE*3.0/4) {
                Robots::look_at(robot_index, PI/2);
			}
...
void Strategy::get_past(int robot_index) {
	past_position[robot_index] = Robots::get_position(robot_index);
	past_transangle[robot_index] = Robots::get_transAngle(robot_index);
}

void Strategy::run(int robot_index) {
