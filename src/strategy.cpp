...
void Strategy::set_roles() {
	if(!transitions_enabled) {
		for(int robotIndex = 0; robotIndex < Robots::SIZE; robotIndex++) {
			switch(Robots::get_role(robotIndex)) {
				case ATTACKER:
					atk = robotIndex;
					break;
	...
	for(int robotIndex = 0; robotIndex < Robots::SIZE; robotIndex++) {
		if(robotIndex == atk || robotIndex == gk) continue;
		def = robotIndex;
		break;
	}
...
void Strategy::set_role(int robotIndex, int role) {
	Robots::set_status(robotIndex, NORMAL_STATE);
	Robots::set_role(robotIndex, role);
	Robots::set_velocity(robotIndex, role);
}

bool Strategy::has_ball(int robotIndex) {
	// Se o robô tem a bola pode levá-la pra frente
	double theta_i_ball = atan2(Ball.y - Robots::get_position(robotIndex).y, Ball.x - Robots::get_position(robotIndex).x);
	return distance(Robots::get_position(robotIndex), Ball) < ABS_ROBOT_SIZE*1.5 //< robô está perto da bola
		   && (abs(Robots::get_orientation(robotIndex) - theta_i_ball)*180/PI < 30 || abs(Robots::get_orientation(robotIndex) - theta_i_ball)*180/PI > 150) //< bola está em uma das frentes
		   && Ball.x > Robots::get_position(robotIndex).x + ABS_ROBOT_SIZE/2; // bola está na frente do atacante em relação ao gol inimigo
    ;
}
...
void Strategy::fixed_position_check(int robotIndex) { // Para posição fixa
	if(Robots::get_fixedPos(robotIndex) && (Robots::get_cmdType(robotIndex) == Robots::CMD::VECTOR || Robots::get_cmdType(robotIndex) == Robots::CMD::POSITION)) {
        Robots::set_velocity(robotIndex, float(Robots::get_default_velocity(robotIndex) * pow(distance(Robots::get_position(robotIndex), Robots::get_target(robotIndex))/(ABS_GOAL_TO_GOAL_WIDTH/4.0), 2)));
		...
		if(Robots::get_velocity(robotIndex) > Robots::get_default_velocity(robotIndex)) Robots::set_velocity(robotIndex);
		if(Robots::get_velocity(robotIndex) < 0.3) Robots::set_velocity(robotIndex, 0.3f);

		if(distance(Robots::get_target(robotIndex), Robots::get_position(robotIndex)) <= fixed_pos_distance) {
			Robots::stop(robotIndex);
		}
	}
}
...
void Strategy::collision_check(int robotIndex) {
    if(Robots::get_fixedPos(robotIndex)) return;

    // se é atacante e está dentro do gol
    if (Robots::get_role(robotIndex) == ATTACKER &&
            Robots::get_position(robotIndex).y > COORD_GOAL_UP_Y && Robots::get_position(robotIndex).y < COORD_GOAL_DWN_Y &&
            Robots::get_position(robotIndex).x < ABS_GOAL_SIZE_X + 0.75 * ABS_ROBOT_SIZE) {

        // verifica se está colidindo
        if(distance(Robots::get_position(robotIndex), past_position[robotIndex]) <= collision_radius) {
            collision_count[robotIndex]++;
        } else {
            get_past(robotIndex);
            collision_count[robotIndex] = 0;
        }

        if(collision_count[robotIndex] >= max_collision_count) {
            ...
            if(distance(Robots::get_position(robotIndex), upper_corner) > distance(Robots::get_position(robotIndex), lower_corner) && Ball.y > COORD_GOAL_MID_Y){
                tempAngle = atan2(sin(past_transangle[robotIndex]+(PI/2)),cos(past_transangle[robotIndex]+(PI/2)));
            } else if (distance(Robots::get_position(robotIndex), upper_corner) < distance(Robots::get_position(robotIndex), lower_corner) && Ball.y < COORD_GOAL_MID_Y) {
                tempAngle = atan2(sin(past_transangle[robotIndex]-(PI / 2)), cos(past_transangle[robotIndex]-(PI / 2)));
            } else {
                tempAngle = Robots::get_transAngle(robotIndex);
            }

            Robots::set_target(robotIndex, cv::Point(Ball.x, Robots::get_position(robotIndex).y));
            Robots::set_command(robotIndex, tempAngle);
        }
    }
    else if(Robots::get_status(robotIndex) != CORNER_STATE && Robots::get_status(robotIndex) != ATK_PENALTI_STATE) {
		if(distance(Robots::get_position(robotIndex), past_position[robotIndex]) <= collision_radius &&
			distance(Robots::get_position(robotIndex), Robots::get_target(robotIndex)) > fixed_pos_distance) {
			collision_count[robotIndex]++;
		} else {
			get_past(robotIndex);
			collision_count[robotIndex] = 0;
		...
		if(collision_count[robotIndex] >= max_collision_count) {
			Robots::set_transAngle(robotIndex, atan2(sin(past_transangle[robotIndex]+PI),cos(past_transangle[robotIndex]+PI)));
		}
	}
}
...
bool Strategy::is_near(int robotIndex, cv::Point point) {
	return distance(Robots::get_position(robotIndex), point) < fixed_pos_distance;
}

void Strategy::position_to_vector(int robotIndex) {
	if(Robots::get_cmdType(robotIndex) != Robots::CMD::VECTOR)
...
double Strategy::potField(int robotIndex, cv::Point goal, int behavior) {
	...
	for (unsigned short i = 0; i < adv.size(); i++) {
		if(distance_meters(Robots::get_position(robotIndex), adv.at(i)) > 0 && distance_meters(Robots::get_position(robotIndex), adv.at(i)) < rep_radius) {
			pot_magnitude[i] = gain_rep*pow(1/(distance_meters(Robots::get_position(robotIndex), adv.at(i)) - 1/rep_radius), 2)/2;
			pot_angle[i] = atan2(double(Robots::get_position(robotIndex).y - adv.at(i).y),- double(Robots::get_position(robotIndex).x - adv.at(i).x));

			if(pot_rotation_decision(robotIndex,goal,adv.at(i)) == 0) {
				pot_magnitude[i] = 0;
			} else if(pot_rotation_decision(robotIndex,goal,adv.at(i))>0) {
	...
	for (int i = 0; i < 3; i++) {
		if(i != robotIndex) {
			if(distance_meters(Robots::get_position(robotIndex), Robots::get_position(i)) < rep_radius) {
				pot_magnitude[j] = gain_rep*pow(1/(distance_meters(Robots::get_position(robotIndex), Robots::get_position(i)) - 1/rep_radius), 2)/2;
				pot_angle[j] = atan2(double(Robots::get_position(robotIndex).y - Robots::get_position(i).y),- double(Robots::get_position(robotIndex).x - Robots::get_position(i).x));
				if(pot_rotation_decision(robotIndex,goal,Robots::get_position(i)) == 0) {
					pot_magnitude[j] = 0;
				} else if(pot_rotation_decision(robotIndex,goal,Robots::get_position(i)) > 0) {
	...
	switch(behavior) {
		case BALL_IS_OBS:
			if(distance_meters(Robots::get_position(robotIndex), Ball) > 0 && distance_meters(Robots::get_position(robotIndex), Ball) < ball_rep_radius) {

				// !TODO Mudar variável para DOUBLE ou fazer cast para FLOAT?
				ball_mag = (gain_rep*pow(1/(distance_meters(Robots::get_position(robotIndex), Ball) - 1/(ball_rep_radius)), 2)/2 );

				// !TODO Mudar variável para DOUBLE ou fazer cast para FLOAT?
				ball_angle = atan2(double(Robots::get_position(robotIndex).y - Ball.y),- double(Robots::get_position(robotIndex).x - Ball.x));
				if(pot_rotation_decision(robotIndex,goal,Ball) == 0) {
					ball_mag = 0;
				} else if(pot_rotation_decision(robotIndex,goal,Ball)>0) {
					// !TODO Mudar variável para DOUBLE ou fazer cast para FLOAT?
		...
		case BALL_ONLY_OBS:
			...
			if(distance_meters(Robots::get_position(robotIndex), Ball) > 0 && distance_meters(Robots::get_position(robotIndex), Ball) < ball_rep_radius) {
				// !TODO Mudar variável para DOUBLE ou fazer cast para FLOAT?
				ball_mag = (gain_rep*pow(1/(distance_meters(Robots::get_position(robotIndex), Ball) - 1/ball_rep_radius), 2)/2.0);
	...
	pot_goalTheta = atan2(double(Robots::get_position(robotIndex).y -goal.y ),- double(Robots::get_position(robotIndex).x - goal.x ));
	pot_goalMag = gain_att * pow(distance_meters(Robots::get_position(robotIndex), goal), 2)/2;
	...
	// pot_theta = robots[robotIndex].orientation - pot_theta;
	// pot_theta = atan2(-sin(robots[robotIndex].orientation - pot_theta),cos(robots[robotIndex].orientation - pot_theta));
	// printf("THETA %f \n",pot_theta*180/PI);
	return pot_theta;
}

int Strategy::pot_rotation_decision(int robotIndex,cv::Point goal, cv::Point obst) {
	int HORARIO = 1;
	int ANTI_HORARIO = -1;
	bool goal_ahead = Robots::get_position(robotIndex).x > obst.x && obst.x > goal.x;
	bool goal_behind = Robots::get_position(robotIndex).x < obst.x && obst.x < goal.x;
	...
	double angle_Obst_Robot = atan2(double(obst.y - Robots::get_position(robotIndex).y), - double(obst.x - Robots::get_position(robotIndex).x));
...
void Strategy::def_wait(int robotIndex) {
	...
	if(distance(Robots::get_position(robotIndex), Robots::get_target(robotIndex)) <= fixed_pos_distance) {
	    Robots::look_at(robotIndex, Ball);
	}
}
...
void Strategy::pot_field_around(int robotIndex) {
	...
	double m2 = double(Robots::get_position(robotIndex).y - Ball_Est.y)/double(Robots::get_position(robotIndex).x - Ball_Est.x);
	...
	if(Robots::get_position(robotIndex).x > Ball.x + ABS_ROBOT_SIZE)
		action1 = false;

	if(is_near(robotIndex, targets_temp) || abs(phi) < beta/2 || action1) {
		v = cv::Point(Robots::get_position(robotIndex).x - Ball.x, Robots::get_position(robotIndex).y - Ball.y);
		...
		Robots::set_transAngle(robotIndex, atan2(double(Robots::get_position(robotIndex).y - Ball.y), - double(Robots::get_position(robotIndex).x - Ball.x)));
		action1 = true;
	}
}
*/
...
void Strategy::crop_targets(int robotIndex) {
	...
	Robots::set_target(robotIndex, target);
}

void Strategy::atk_routine(int robotIndex) {
	cv::Point target;
	cv::Point goal = cv::Point(COORD_GOAL_ATK_FRONT_X, COORD_GOAL_MID_Y);

	double m1 = double(goal.y - Ball.y)/double(goal.x - Ball.x);
	double m2 = double(Robots::get_position(robotIndex).y - Ball.y)/double(Robots::get_position(robotIndex).x - Ball.x);
	double phi = atan((m2 - m1) / (1 + m2 * m1));

	Robots::set_velocity(robotIndex);

	if(Robots::get_status(robotIndex) != ATK_PENALTI_STATE) {
		if((Robots::get_position(def).x < COORD_MID_FIELD_X // se o defensor tá atrás do meio de campo
			...
			||
			(has_ball(robotIndex) && // atacante tem a posse de bola
			abs(phi)*180/PI < 30)) { // angulo com o gol tá bom
			Robots::set_status(robotIndex, ATK_PENALTI_STATE); // chuta
		}
			...
			Robots::set_status(robotIndex, CORNER_STATE); // a bola tá no corner, gira
		}
		else if(Ball.x < def_corner_line) { // se a bola tá antes da nossa linha x da área
			Robots::set_status(robotIndex, DEF_CORNER_STATE); // a bola tá no nosso corner
		}
			...
			Robots::set_status(robotIndex, SIDEWAYS); // sideways
		} else {
			Robots::set_status(robotIndex, NORMAL_STATE); // senão, age normalmente
		}
	}

	switch(Robots::get_status(robotIndex)) {
		case NORMAL_STATE:
			// leva a bola pro gol deles
			Robots::set_cmdType(robotIndex, Robots::CMD::VECTOR);
			// pot_field_around(robotIndex);
			uvf(robotIndex);
		...
		case SIDEWAYS:
			...
			Robots::set_cmdType(robotIndex, Robots::CMD::POSITION);

			// empurra a bola no canto mesmo
			if(Ball.y > ABS_FIELD_HEIGHT - ABS_ROBOT_SIZE * 1.5) { // se a bola tá na lateral inferior
				if(distance(Robots::get_position(robotIndex), Ball) > ABS_ROBOT_SIZE // se a bola tá longe do robô
					|| Ball.x >= Robots::get_position(robotIndex).x + ABS_ROBOT_SIZE/1.5) { // ou se a bola não tá travada por ele
				    target = Ball;
				}
				else { // se a bola tá travada
					Robots::spin_counter_clockwise(robotIndex, 0.8);
				}

				if(Ball.x < Robots::get_position(robotIndex).x) { // se a bola tá atrás do atacante
				...
				if(distance(Robots::get_position(robotIndex), Ball) > ABS_ROBOT_SIZE // se a bola tá longe do robô
					|| Ball.x >= Robots::get_position(robotIndex).x + ABS_ROBOT_SIZE/1.5) { // ou se a bola não tá travada
                    target = Ball;
				}
				else { // se a bola tá travada
					Robots::spin_clockwise(robotIndex, 0.8);
				}
		...
		case CORNER_STATE:
			...
			if(Ball.y < COORD_GOAL_MID_Y) { // se a bola tá acima do gol
				if(Ball.y < COORD_GOAL_UP_Y) { // acima ou abaixo da trave, escolher o giro para levar a bola para o gol ou para faze-la entrar
					if(Ball.x > Robots::get_position(robotIndex).x && distance(Ball, Robots::get_position(robotIndex)) < ABS_ROBOT_SIZE)
						Robots::spin_clockwise(robotIndex, 0.8); // giro que leva a bola ao gol
					else
						target = Ball; // faz a bola travar no canto pra girar certo
				} else {
					if(Ball.y > Robots::get_position(robotIndex).y && distance(Ball, Robots::get_position(robotIndex)) < ABS_ROBOT_SIZE)
						Robots::spin_counter_clockwise(robotIndex, 0.8); // giro para faze-la entrar
					else
						target = Ball;
				}
			} else {
				if(Ball.y > COORD_GOAL_DWN_Y) { // se a bola tá abaixo do gol
					...
					if(Ball.y > Robots::get_position(robotIndex).y && distance(Ball, Robots::get_position(robotIndex)) < ABS_ROBOT_SIZE)
						Robots::spin_clockwise(robotIndex, 0.8); // giro para faze-la entrar
					else
						target = Ball;
				}
			}
		...
		case DEF_CORNER_STATE:
			...
			if(Ball.y > COORD_GOAL_MID_Y) {
				...
				if(distance(Ball, Robots::get_position(robotIndex)) < ABS_ROBOT_SIZE && Ball.y > Robots::get_position(robotIndex).y)
					Robots::spin_counter_clockwise(robotIndex, 0.8);
				// if(distance(Robots::get_position(i), robots[i].target) <= fixed_pos_distance) robots[i].target = Ball;
				if(is_near(robotIndex, target) && Ball.y > Robots::get_position(robotIndex).y) {
					target = Ball;
				}
		...
		case ATK_PENALTI_STATE:
			...
			Robots::set_cmdType(robotIndex, Robots::CMD::VECTOR);
            Robots::kick(robotIndex, goal);

			if(!has_ball(robotIndex))
				Robots::set_status(robotIndex, NORMAL_STATE);
	}
	//crop_targets(robotIndex);
}

void Strategy::def_routine(int robotIndex) {
	cv::Point target;
	cv::Point goal = cv::Point(COORD_GOAL_ATK_FRONT_X, COORD_GOAL_MID_Y);
	...
	if(Robots::get_position(robotIndex).x > COORD_MID_FIELD_X && abs(phi)*180/PI < 30) {
		Robots::set_status(robotIndex, ATK_PENALTI_STATE);
	}
	else if(Robots::get_position(atk).x > Ball.x
		&& Ball.x < COORD_MID_FIELD_X
		&& distance(Robots::get_position(atk), Ball) > 3*ABS_ROBOT_SIZE) {
		Robots::set_status(robotIndex, STEP_BACK);
	}
	else if(Ball.x < def_corner_line) {
		Robots::set_status(robotIndex, DEF_CORNER_STATE);
	}
	else {
		Robots::set_status(robotIndex, NORMAL_STATE);
	}
	...
	switch(Robots::get_status(robotIndex)) {
		case NORMAL_STATE:
			...
			if(Ball_Est.x > corner_atk_limit) {
				...
				Robots::set_target(robotIndex, target); // deve ser chamado antes do wait
				def_wait(robotIndex);
			}
		...
		case STEP_BACK:
			...
			if(Ball.x > Robots::get_position(robotIndex).x + ABS_ROBOT_SIZE) {
				Robots::set_target(robotIndex, Ball);
			}
			else {
				// pot_field_around(robotIndex);
				crop_targets(robotIndex);
		...
		case DEF_CORNER_STATE:
			...
			if(Ball.y > COORD_GOAL_MID_Y) {
				...
				if(distance(Ball, Robots::get_position(robotIndex)) < ABS_ROBOT_SIZE && Ball.y > Robots::get_position(robotIndex).y)
					Robots::spin_counter_clockwise(robotIndex, 0.8);
				// if(distance(Robots::get_position(robotIndex), Robots::get_target(robotIndex)) <= fixed_pos_distance) robots[robotIndex].target = Ball;
				if(is_near(robotIndex, target) && Ball.y > Robots::get_position(robotIndex).y) {
					target = Ball;
				}
			} else {
		...
		case ATK_PENALTI_STATE:
			...
			Robots::set_cmdType(robotIndex, Robots::CMD::VECTOR);
            Robots::kick(robotIndex, Ball);
    }
}

void Strategy::gk_routine(int robotIndex) {
	switch(Robots::get_status(robotIndex)) {
		case NORMAL_STATE:
			Robots::set_fixedPos(robotIndex, true);
			...
			if(distance(Ball, Ball_Est) > ABS_ROBOT_SIZE && Ball.x > Ball_Est.x && (Ball.x > COORD_BOX_DEF_X)) {
				...
				if(Ball.x > COORD_MID_FIELD_X) {
					...
					double approach = abs(Ball.y - pred_y) * (Ball.x - (Robots::get_position(robotIndex).x + ABS_ROBOT_SIZE))/float(COORD_MID_FIELD_X - Robots::get_position(robotIndex).x);
			...
			if(distance(Robots::get_position(robotIndex), Ball) < ABS_ROBOT_SIZE*1.3) {
				if(Ball.y <= COORD_GOAL_UP_Y) {
					Robots::spin_clockwise(robotIndex, 1.f);
				} else if(Ball.y >= COORD_GOAL_DWN_Y) {
					Robots::spin_counter_clockwise(robotIndex, 1.f);
				}
			}

			if(distance(Robots::get_position(robotIndex), Ball) < ABS_ROBOT_SIZE*1.3) {
				if(Ball.y <= Robots::get_position(robotIndex).y) {
					Robots::spin_clockwise(robotIndex, 1.f);
				} else if(Ball.y >= Robots::get_position(robotIndex).y){
					Robots::spin_counter_clockwise(robotIndex, 1.f);
				}
			}
			// chute pra tirar a bola da defesa
			if(abs(Ball.x - Robots::get_position(robotIndex).x) <= ABS_ROBOT_SIZE && Ball.y < COORD_BOX_DWN_Y && Ball.y > COORD_BOX_UP_Y) {
				if (Ball.y <= Robots::get_position(robotIndex).y) {
					target.y = 0;
					target.x = goalie_line; //  + ABS_ROBOT_SIZE/2
					Robots::set_velocity(robotIndex, MAX_VEL);
				} else {
					target.y = 480;
					target.x = goalie_line; //  + ABS_ROBOT_SIZE/2
					Robots::set_velocity(robotIndex, MAX_VEL);
			...
			if(distance(Robots::get_position(robotIndex), Robots::get_target(robotIndex)) < ABS_ROBOT_SIZE*3.0/4) {
                Robots::look_at(robotIndex, PI/2);
			}
...
void Strategy::get_past(int robotIndex) {
	past_position[robotIndex] = Robots::get_position(robotIndex);
	past_transangle[robotIndex] = Robots::get_transAngle(robotIndex);
}
...
double Strategy::hyper_spiral_fih(cv::Point p, bool clockwise, int robotIndex) {
	...
	if(ro > radiusSpiral) {
		a = (PI / 2.0) * (2.0 - (radiusSpiral + kr) / (ro + kr));
	} else {
		a = (PI / 2.0) * sqrt(ro/radiusSpiral);
	}

	if(clockwise) return wrap_to_pi(theta + a);
	else return wrap_to_pi(theta - a);
}

std::pair<double, double> Strategy::hyper_spiral_nh(cv::Point p, bool clockwise, int robotIndex) {
	double fi = hyper_spiral_fih(p, clockwise, robotIndex);
	double cosine = cos(fi);
	double sine = sin(fi);
	return std::make_pair(cosine, sine);
}

double Strategy::wrap_to_pi(double theta) {
	if(theta > PI) return theta - 2*PI;
	else if(theta < -PI) return theta + 2*PI;
	else return theta;
}

double Strategy::angle_with_x(cv::Point p) {
	double x = 1.0, y = 0.0;
	double theta = atan2(x*p.y - y*p.x, x*p.x + y*p.y);
	return theta;
}