...
// Aloca espalo de mem�ria para a matriz
...
// Aloca espalo de mem�ria para o vetor
...
// Copia o conte�do da matriz A e do vetor B para a matriz M
...
void LS::retrosubstitutionU( double** M, double* R, int m){
    ...
    // Este loop Sobe a matriz escalonada achando o valor de cada vari�vel
    for (int i = m-1; i >= 0; i--){

        // Preenche o vetor R com B
        R[i] = M[i][m];

        // Subtrai de R todos os valores ponderados entre B e o piv�
        for (int j = (m-1); j > i; j--)
            R[i] -= M[i][j]*R[j];
...
void LS::lu( double** A, double** L, double** U, int m){
    ...
    double sum = 0; // Vari�vel auxiliar que armazena os valores a serem subtra�dos a cada passo

    // A cada passada deste la�o, define-se uma linha de U e uma coluna de L
    for (int i = 0; i < m; i++){

        // A cada passada deste la�o, define-se uma linha de U
        for (int j = i; j < m; j++){
            sum = 0;
            for (int k = 0; k < i; k++)
                sum += L[i][k]*U[k][j];
        ...
        // A cada passada deste la�o, define-se uma coluna de L
        for (int j = i+1; j < m; j++){
            sum = 0;
            for (int k = 0; k < j; k++)
                sum += L[j][k]*U[k][i];
